...
from billingclient.openstack.common import cliutils
...
class BillingShell(object):

    def get_base_parser(self):
        parser = argparse.ArgumentParser(
            prog='billing',
            description='',
            epilog='See "billing help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version=billingclient.__version__)

        parser.add_argument('-d', '--debug',
                            default=bool(cliutils.env('BILLINGCLIENT_DEBUG')
                                         ),
                            action='store_true',
                            help='Defaults to env[BILLINGCLIENT_DEBUG].')
        ...
        parser.add_argument('--billing-url', metavar='<BILLING_URL>',
                            dest='os_endpoint',
                            default=cliutils.env('BILLING_URL'),
                            help=("DEPRECATED, use --os-endpoint instead. "
                                  "Defaults to env[BILLING_URL]."))

        parser.add_argument('--billing_url',
                            dest='os_endpoint',
                            help=argparse.SUPPRESS)

        parser.add_argument('--billing-api-version',
                            default=cliutils.env(
                                'BILLING_API_VERSION', default='1'),
                            help='Defaults to env[BILLING_API_VERSION] '
                            'or 1.')

        parser.add_argument('--billing_api_version',
                            help=argparse.SUPPRESS)
    ...
    def parse_args(self, argv):
        ...
        api_version = options.billing_api_version
    ...
    def no_project_and_domain_set(args):
        if not ((args.os_project_id or args.os_project_name) or
    ...
    def do_bash_completion(self, args):
        ...
        The billing.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main(args=None):
    try:
        ...
        print("Stopping Billing Client", file=sys.stderr)
        sys.exit(130)
