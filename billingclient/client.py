...
from billingclient.openstack.common.apiclient import auth
from billingclient.openstack.common.apiclient import exceptions
...
def _get_endpoint(ks_session, **kwargs):
    ...
    service_type = kwargs.get('service_type') or 'billing'
...
class AuthPlugin(auth.BaseAuthPlugin):
    opt_names = ['tenant_id', 'region_name', 'auth_token',
                 ...
                 'project_id', 'domain_id', 'domain_name',
    ...
    def _do_authenticate(self, http_client):
        ...
        if not (token and endpoint):
            ...
            ks_kwargs = {
                ...
                'domain_id': self.opts.get('domain_id'),
                'domain_name': self.opts.get('domain_name'),
    ...
    def sufficient_options(self):
        ...
        has_tenant_or_domain = (self.opts.get('tenant_id') or
                                self.opts.get('tenant_name') or
                                self.opts.get('project_id') or
                                self.opts.get('project_name') or
                                self.opts.get('domain_name') or
                                self.opts.get('domain_id') or
                                self.opts.get('user_domain_name') or
                                self.opts.get('user_domain_id'))

        has_credential = (self.opts.get('username') and has_tenant_or_domain
...
def get_client(version, **kwargs):
    ...
    :param kwargs: keyword args containing credentials, either:
            ...
            * os_endpoint: Billing API endpoint
            or:
            ...
            * os_domain_name: domain name for the user project
            * os_domain_id: domain id for the user project
    ...
    cli_kwargs = {
        ...
        'domain_name': kwargs.get('os_domain_name'),
        'domain_id': kwargs.get('os_domain_id'),
